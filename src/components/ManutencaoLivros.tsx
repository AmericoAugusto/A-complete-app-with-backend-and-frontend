...
const ManutencaoLivros: React.FC<AboutBooks> = () => {
    ...
    const filtrarLista = async (campos: any) => {
        try {
            const lista = await inAxios.get(`livros/filtro/${campos.id}`);
            lista.data.lenght
            ? setLivros(lista.data)
    ...
    const excluir = async (id: number, titulo: string ) => {
        ...
        const alterar = async (id: number, titulo: string, index: any) => {
                            ...
                            <input type="text"
                             className="form-control"
                              placeholder="Título ou autor"
                               required {...register("palavra")} />
                               <input type="submit" className="btn btn-priamry" value="Pesquisar" />
                               <input type="button"
                                className="btn btn-danger"
                                 value="Todos" onClick={() => {reset({palavra: ""}); obterLista();}} />
                        </div>
                    </form>
                </div>
            </div>
            <table className="table table-striped mt-3">
                ...
                <tbody>
                    {livros.map((livro, index) => (
                        ...
                        foto={livro.foto} excluirClick={() => excluir(livro.id, livro.titulo, index)} />
                    ))}
                </tbody>
            </table>
        </div>
